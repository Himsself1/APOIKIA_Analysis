...
metadata <- read.table(
  file = "~/apoikia/APOIKIA_Analysis/qpadm_after_revisions/metadata_qpadm/qpAdm_most_proximate_ammotopos_model1.tsv",
...
all_target_indexes <- grep("TARGET", metadata_all_targets[,6])
additional_outgroups <- grep("IRN_GanjDareh_N_8300-7600BCE|SRB_M_HG_IronGates_9800-5700BCE", metadata_all_targets[,5])

metadata[c(all_target_indexes,additional_outgroups),c(5,6)] <- metadata_all_targets[c(all_target_indexes,additional_outgroups),c(5,6)]
...
most_prox_ind_file <- paste0(c(ind_file, ".most_proximate_ammotopos"), collapse = '')
...
write.table(
  qpwave_pairs_out,
  "~/apoikia/qpadm_results/qpwave_results/qpwave_pairs_most_proximate_ammotopos.tsv",
  quote = F, sep = '\t', row.names = F
)

write.table(
  qpwave_high_pvalues,
  "~/apoikia/qpadm_results/qpwave_results/qpwave_pairs_most_proximate_ammotopos_high_pvalues.tsv",
  quote = F, sep = '\t', row.names = F
)

## Does it need more right???

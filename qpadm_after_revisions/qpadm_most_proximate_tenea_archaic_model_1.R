...
metadata <- read.table(
  file = "~/apoikia/APOIKIA_Analysis/qpadm_after_revisions/metadata_qpadm/qpAdm_most_proximate_tenea_archaic_model1.tsv",
...
qpwave_pairs_out <- qpwave_pairs(
  ...
  left = setdiff(
    ...
    unique( metadata_filtered[indexes_of_apoikia_non_target,5] )
  ) # Remove apoikia populations that might be labeled as sources.
...
write.table(
  qpwave_pairs_out,
  "~/apoikia/qpadm_results/qpwave_results/qpwave_pairs_most_proximate_tenea_archaic.tsv",
  quote = F, sep = '\t', row.names = F
)

write.table(
  qpwave_high_pvalues,
  "~/apoikia/qpadm_results/qpwave_results/qpwave_pairs_most_proximate_tenea_archaic_high_pvalues.tsv",
  quote = F, sep = '\t', row.names = F
)

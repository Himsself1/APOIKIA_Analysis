...
yoruba_output <- "/home/aggeliki/apoikia/f3_outputs/f3_results/dataset_25_27_2025/variability/yoruba_outgroup"
yoruba_plot <- "/home/aggeliki/apoikia/f3_outputs/plots/dataset_25_27_2025/variability/yoruba_outgroup"
...
colnames(all_f3_ammotopos) <- c(colnames(temp_f3_initial), "SNPs")  
...
colnames(all_f3_amv_archaic) <- c(colnames(temp_f3_initial), "SNPs")

# ** Archaic Tenea has only 1 individual
...
colnames(all_f3_amv_classical) <- c(colnames(temp_f3_initial), "SNPs")
...
colnames(all_f3_amv_hel) <- c(colnames(temp_f3_initial), "SNPs")
...
colnames(all_f3_tenea_hel) <- c(colnames(temp_f3_initial), "SNPs")
...
colnames(all_f3_tenea_roman) <- c(colnames(temp_f3_initial), "SNPs")
...
plotted_boxes <- ggplot(data_boxes_to_plot,
                        aes( x = location, y = est, fill = age, color = age,
                            group = interaction(location, age) ))
plotted_boxes <- plotted_boxes + geom_boxplot(
  position = position_dodge2(width = 0.75, preserve = "single"),
  outliers = F )
plotted_boxes <- plotted_boxes + ylab( "F3" )
## plotted_boxes <- plotted_boxes + geom_dotplot(
##   binaxis = "y", stackdir = "center", dotsize = 0.5, stackgroups = T, binpositions = "all",
##   position = position_dodge( width = 0.65, preserve = "total"),
##   show.legend = F
## )
plotted_boxes <- plotted_boxes + geom_point(

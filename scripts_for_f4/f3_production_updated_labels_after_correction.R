...
flags <- read.table("~/apoikia/APOIKIA_Analysis/scripts_for_f4/Updated_Labels_for_f3_after_correction.csv",
...
new_pops_tenea_hel <- c() ## Population Names need to be corrected
for (i in 1:length(apoikia_dataset$ind[[1]])) {
  if (apoikia_dataset$ind[[1]][i] %in% flags$Genetic_ID) {
    temp_index <- which(flags$Genetic_ID %in% apoikia_dataset$ind[[1]][i])
    new_pops_tenea_hel <- c(new_pops_tenea_hel, flags$Hellenistic.325.100BCE.[temp_index])
  } else {
    new_pops_tenea_hel <- c(new_pops_tenea_hel, apoikia_dataset$ind[[3]][i])
  }
}
...
f3_tenea_hellenistic <- do.call(rbind, lapply(
  ...
  function(x) {
    score_3(
      all_combinations[x, ],
      new_pops_tenea_hel[-indexes_to_ignore],
      apoikia_dataset$ind[[1]][-indexes_to_ignore],
      "~/apoikia/EIGENSTRAT/dataset_04_05_2024/APOIKIA_PLUS_PUBLIC_ANCIENT/apoikia_with_yoruba"
    )
  }

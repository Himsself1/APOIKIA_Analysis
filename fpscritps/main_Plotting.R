...
runs_from_file <- runs_from_file[grepl("^haplo", runs_from_file)]  # Match files starting with 'haplo' 
# Step 2: Get the list of files that match the criteria
# Modify the grep pattern to include just "haplo"
files <- list.files(pattern = ".*filtered.OUTQPADM$")
files <- files[grepl("^haplo", files)]  # Match files starting with 'haplo' or 'diplo'

print(runs_from_file)
...
# custom_palette <- c("#0372D3","#330078") # for amv_spec run
...
long_data$Run <- gsub("haplo-missing(\\d+)-afprodFALSE", "H/\\1/F", long_data$Run)
long_data$Run <- gsub("haplo-missing(\\d+)-afprodTRUE", "H/\\1/T", long_data$Run)
...
save_plot <- function(output_file, plot, num_outer_pops) {
  ...
  base_width <- base_width + (0.0025 * num_outer_pops)
  ...
  ggsave(output_file, plot = plot, width = base_width, height = plot_height, dpi = 100, limitsize = FALSE)
}
